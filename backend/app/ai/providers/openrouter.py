...
class OpenRouterProvider(AIProvider):
    """AI provider backed by OpenRouter via the OpenAI-compatible API."""
    ...
    def _map_model_name(self, friendly_name: str) -> str:
        ...
        return mapping.get(friendly_name, "google/gemini-2.5-flash")
    ...
    async def _generate_with_retry(
        ...
        """
        Send a request to OpenRouter with progressive retry logic and schema validation.
        """
        ...
        for attempt in range(1, 4):
            logger.info(f"OpenRouter {operation_name} call - Attempt {attempt}/3")
            try:
                # API request call
                response = await self.client.chat.completions.create(
                    model=self.model,
                ...
                # OpenRouter json_object format may wrap arrays inside generic objects
                ...
                if any(phrase in err_lower for phrase in [
                    ...
                    logger.error(f"Non-retryable OpenRouter error detected. Aborting retries.")
                    break
        ...
        async with AsyncSessionLocal() as session:
            event = SystemEvent(
                ...
                severity="error",
                message=f"OpenRouter failed 3 attempts for {operation_name}: {last_error}",
    ...
    async def analyze_document(
        ...
        try:
            ...
            logger.error(f"OpenRouter analyze_document failed: {exc}")
    ...
    async def generate_quiz(
        ...
        try:
            ...
            logger.error(f"OpenRouter generate_quiz failed: {exc}")
            raise AIProviderError(f"Quiz generation failed: {exc}") from exc

    async def embed(self, texts: list[str]) -> list[list[float]]:
        """OpenRouter does not provide embeddings — delegate to GeminiEmbedder."""
    ...
    async def evaluate_answers(
        ...
        """Basic evaluation using OpenRouter chat."""
        lines = []
        ...
        try:
            ...
            logger.warning(f"OpenRouter evaluate_answers failed: {exc}")
            return EvaluationResult(
                per_question={},
